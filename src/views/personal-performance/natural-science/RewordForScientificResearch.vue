  ...
  <div>
    <v-card>
      <v-toolbar flat color="white">
        <v-toolbar-title>科研奖励</v-toolbar-title>
        <v-spacer></v-spacer>
        <v-spacer></v-spacer>
      </v-toolbar>
      <v-card>
        <v-toolbar flat color="white">
          <v-toolbar-title>科研奖励信息录入</v-toolbar-title>
          <v-spacer></v-spacer>
          <v-spacer></v-spacer>
        </v-toolbar>
...
export default {
  ...
  data() {
    return {
      valid: false,
      validRules: {
        prizeWinner: [v => !!v || "请填写获奖者姓名"],
        numberOfPathers: [v => v !== "" || "请选择合作单位数"],
        prizeLevel: [v => v !== "" || "请选择奖励等级"],
        schookRanking: [v => v !== "" || "请选择校排名"],
        rewordPlaces: [v => v !== "" || "请选择奖励名次"]
      },
      defaultForm: {
        prizeWinner: "",
        numberOfPathers: "",
        ...
        <v-form ref="form" v-model="valid">
          <v-container>
            <v-layout row wrap justify-space-around>
              <v-flex md5>
                <v-text-field
                  v-model="defaultForm.prizeWinner"
                  :rules="validRules.prizeWinner"
                  label="获奖者"
                  required
                ></v-text-field>
              </v-flex>
              <v-flex md5>
                <v-select
                  v-model="defaultForm.numberOfPathers"
                  :rules="validRules.numberOfPathers"
                  :items="numberOfPathersItems"
                  item-text="state"
                  item-value="abbr"
                  label="合作单位数"
                  persistent-hint
              ...
              <v-flex md5>
                <v-select
                  v-model="defaultForm.prizeLevel"
                  :rules="validRules.prizeLevel"
                  :items="prizeLevelItems"
                  item-text="state"
                  item-value="abbr"
                  label="奖励等级"
                  persistent-hint
              ...
              <v-flex md5>
                <v-select
                  v-model="defaultForm.schookRanking"
                  :rules="validRules.schookRanking"
                  :items="schookRankingItems"
                  item-text="state"
                  item-value="abbr"
                  label="我校排名"
                  persistent-hint
              ...
              <v-flex md5>
                <v-select
                  v-model="defaultForm.rewordPlaces"
                  :rules="validRules.rewordPlaces"
                  :items="rewordPlacesItems"
                  item-text="state"
                  item-value="abbr"
                  label="奖励名次"
                  persistent-hint
